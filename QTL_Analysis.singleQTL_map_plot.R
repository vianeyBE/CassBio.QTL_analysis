# The function create PDF files with the chromosome, interval and LOD profile
...
plot.single.QTL = function(qtldata, int_method){
  ...
  for (LG in unique(qtldata$chr)){
    ...
    # Plots a basic plot with the chromosome, the SNPs, QTL intervals, and labels 
    # of flank SNPs inside the chromosome
    qtlmap.basic = function(chr.size.qtl, data.map.qtl, data.qtl){
      qtl.map = 
        ...
        scale_y_reverse(name="Location (cM)", 
                        expand = expansion(mult = c(0, 0)),
                        breaks = my_breaks,
                        labels = my_labs
        ) +
        geom_rect(data=chr.size.qtl, #Draw chromosomes
                  aes(xmin=1-wide.chr/3,
                      xmax=1+wide.chr/3,
                      ymin=0, 
                      ymax=size+2), 
                  alpha = 0, 
        ...
        geom_rect(data=data.map.qtl, #Draw SNPs
        ...
        geom_rect(data=data.qtl, #Draw qtl intervals
        ...
        theme(panel.background = element_blank(), # Theme
              ...
              axis.text.y = element_text(size = 24, face = "bold", color = "black", margin=margin(0,0,0,20)),
    ...
    for ( i in 1:dim(data.qtl)[1] ){
      ...
      q <- geom_richtext(data = data.qtl[i,],
                         aes(x = xvalue, 
                             y = end.pos, # it might be necessary to move label (!)
    ...
    # LOD Profile --------------------------------------------------------------
    qtl.lod.temp = data.frame(loci =  row.names(out),
    ...
    plotLODmqm = function(data.lod.temp){
        ...
        plot.lod= 
          ...
          theme_classic() + 
          theme(
            strip.text = element_text(size= 15, face = "bold", family=font),
          ...
          ) + xlab("") + ylab("") + scale_color_manual(values = mypal.qtl)
    ...
    save_plot(plot=qtls.plot, 
              filename = paste(outputplot, prefixResults, ".single_qtl.", LG, ".", int_method, ".pdf", sep = ""),
              base_height = 30)
  }
