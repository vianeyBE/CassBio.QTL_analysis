# Short name: Plots for Rqtl
# Description: Creates PDF files with 
...
genoPlot <- function(cross){
  ...
  # Plot
  genotype.plot <- ggplot(data, aes(x = marker, y = as.factor(Individual))) + 
      ...
      labs(x = "Makers", y = "Individuals", fill = "Genotypes") +
...
missGenoPlot <- function(cross){
  ...
  missing.plot <- ggplot(data, aes(x = marker, y = as.factor(Individual))) +
...
plotPhenotypes.pdf <- function(data.pheno){
    ...
    # tmp <- 
      ...
      # Aesthetic
        theme(panel.background = element_blank(),
              ...
              axis.text.y = element_text(face = "bold", family = font, color = "black",
  ...
  # save_plot(plot = phenotypes.all, filename = paste(outputplot, prefixResults,
  #                  ".phenotype-all.pdf", sep = ""), base_height = 30)
...
plotPhenotypes <- function(data.pheno){
  ...
  for (i in 2:length(phenotypes)){
    print(ggplot(data.pheno, aes_string(x = data.pheno[,i])) + 
        geom_histogram(aes(y = ..density..), colour = "black", fill = "white") +
        geom_density(alpha = .2, fill = "#FF6666", size = 1, color = "#FF6666") + 
...
plotLODCore <- function(data.lod.temp){
  ...
  plot.lod <- ggplot(data.lod, aes(x = pos, y = value)) + 
    ...
    theme(# panel.background = element_blank(),
      ...
      axis.title.y.right = element_blank(), axis.ticks.y.right = element_blank(),
      axis.ticks.y.left = element_line(size = 1), axis.text.y.right = element_blank(),
      strip.text = element_text(face = "bold", family = font),
...
plotLOD <- function(out){ # Plot all phenotypes
  ...
  levelsLG <- c("LG1", "LG2", "LG3", "LG4", "LG5", "LG6", "LG7", "LG8", "LG9", "LG10",
                "LG11", "LG12",  "LG13",  "LG14",  "LG15", "LG16", "LG17","LG18")
  levelschr <- c(1:18)
